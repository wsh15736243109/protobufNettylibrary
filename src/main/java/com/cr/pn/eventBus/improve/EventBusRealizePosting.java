...
import com.cr.pn.eventBus.asyEvent.AsyEventRequestPosting;
 ...
 * 消息接收为发布线程.
 */
public class EventBusRealizePosting<T extends BeanBase> extends EventBusRealizeBase<T> {

    private AsyEventRequestPosting<T> asyEventRequest;

    public EventBusRealizePosting(BaseReceiveEventBus<T> receiveEventBus){
     ...
     * 发送方是否能够接收到消息
     */
    public EventBusRealizePosting(BaseReceiveEventBus<T> receiveEventBus,boolean isSendMy){
        super(receiveEventBus,isSendMy);
        asyEventRequest = new AsyEventRequestPosting<>(this);
        register();
    }
