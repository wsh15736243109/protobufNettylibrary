...
import com.cr.pn.eventBus.asyEvent.AsyEventRequestMain;
 ...
 * 消息接收为发布线程.
 */
public class EventBusRealizeMain<T extends BeanBase> extends EventBusRealizeBase<T> {


    private AsyEventRequestMain<T> asyEventRequest;

    public EventBusRealizeMain(BaseReceiveEventBus<T> receiveEventBus){
     ...
     * 发送方是否能够接收到消息
     */
    public EventBusRealizeMain(BaseReceiveEventBus<T> receiveEventBus,boolean isSendMy){
        super(receiveEventBus,isSendMy);
        asyEventRequest = new AsyEventRequestMain<>(this);
        register();
    }
    ...
    public void onReceiveEvent(T t) {
        if (t.getClass() == PostEvent.class){
            T t1 = ((PostEvent)t).getT();
            if (IsSendMsg(t1)){
                receiveEventBus.onReceiveEvent(t1);
            }
        }
        if (t.getClass() == PostStickyEvent.class){
            T t1 = ((PostStickyEvent)t).getT();
            if (IsSendMsg(t1)){
                receiveEventBus.onReceiveEvent(t1);
            }
        }
    ...
    public void onReceiveStickyEvent(T t) {
        if (t.getClass() == PostStickyEvent.class){
            T t1 = ((PostStickyEvent)t).getT();
            if (IsSendMsg(t1)){
                receiveEventBus.onReceiveStickyEvent(t1);
            }
        }
    }
