...
import com.cr.pn.eventBus.asyEvent.AsyEventRequestBack;
 ...
 * 消息接收为发布线程.
 */
public class EventBusRealizeBack<T extends BeanBase> extends EventBusRealizeBase<T> {

    private AsyEventRequestBack<T> asyEventRequest;

    public EventBusRealizeBack(BaseReceiveEventBus<T> receiveEventBus){
     ...
     * 发送方是否能够接收到消息
     */
    public EventBusRealizeBack(BaseReceiveEventBus<T> receiveEventBus, boolean isSendMy){
        super(receiveEventBus,isSendMy);
        asyEventRequest = new AsyEventRequestBack<>(this);
        register();
    }
